...
//    seed = mrb_fixnum_value((mrb_int)(time(NULL) + mt_rand(t)));
    seed = mrb_fixnum_value((mrb_int)(mrb_fixnum(mrb_mruby_get_utim(mrb)) + mt_rand(t)));
...
get_random(mrb_state *mrb) {
  ...
  return mrb_const_get(mrb,
             mrb_obj_value(mrb_class_get_under(mrb, ev3rt, "Random")),
...
static mrb_value
mrb_mruby_random_init(mrb_state *mrb, mrb_value self)
{
  mrb_value seed;
  mt_state *t;
...
void mrb_mruby_random_gem_init(mrb_state *mrb)
{
	...
	struct RClass * random = mrb_define_class_under(mrb, ev3rt_class, "Random", mrb->object_class);
//	struct RClass *array = mrb->array_class;
  ...
  mrb_define_method(mrb, random, "initialize", mrb_mruby_random_init, MRB_ARGS_OPT(1));
